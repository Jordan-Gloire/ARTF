...
import { extractValueOptimizedForRow } from "@/src/hooks/app_hooks";
  ...
  const customColums = useMemo<ColumnDef<TData>[]>(
    () =>
      columns.map((col, index) => ({
        ...col,
        // cell: (info: any) => extractValueOptimizedForRow(info.getValue()),
      })),
    [columns]
  );
      ...
      <div className="flex w-full items-center justify-between">
        <div className="flex items-center gap-2">
          {form.length > 0 && (
            <DialogForm
              trigger={
                <Button size="lg" className="font-semibold" variant="default">
                  Ajouter
                </Button>
              }
              title={props.tittle ?? ""}
